...
india.red <- readOGR('C:/Users/Abinand Reddy/Desktop/Thesis/NCF/eBird/IndiaStates_2011','IndiaStates_2011')
...
current.month = '2017-11'

region.list <- all %>% filter(upload.month == current.month)

  
region.peeps <- region.list %>% group_by(SAMPLING.EVENT.IDENTIFIER) %>% slice(1) %>% ungroup() %>% group_by(SUBNATIONAL1_CODE,FULL.NAME) %>% 
      summarise(no.lists = n_distinct(SAMPLING.EVENT.IDENTIFIER),
                no.hrs = round(sum(DURATION.MINUTES, na.rm=TRUE)/60, 1)) %>% arrange(desc(no.lists))
  
region.stats <- region.list %>% group_by(SUBNATIONAL1_CODE) %>% summarise(total.recs = n(), 
...
region.spp <- region.list %>% filter(ALL.SPECIES.REPORTED == 1) %>% group_by(SUBNATIONAL1_CODE) %>% mutate(visits = n_distinct(group.id)) %>% group_by(SUBNATIONAL1_CODE,SPECIES.NAME) %>%
      summarise(perc.lists = round(100 * n_distinct(group.id)/max(visits), 0)) %>%
      arrange(desc(perc.lists)) %>% slice(1:10)
  


rel.states = c('IN-KL','IN-MH','IN-CT','IN-MP','IN-GA','IN-TN','IN-WB','IN-KA')

for (current.state in rel.states){

region.list.prev <- all %>% filter(upload.month < current.month, SUBNATIONAL1_CODE == current.state) %>% group_by(group.id) %>% select(LONGITUDE,LATITUDE) %>% slice(1)
    
region.list.loc <- region.list %>% filter(SUBNATIONAL1_CODE == current.state) %>% group_by(group.id) %>% select(LONGITUDE,LATITUDE) %>% slice(1)
    

disp.month = paste(month.name[as.numeric(strsplit(current.month, split = '-')[[1]][2])],strsplit(current.month, split = '-')[[1]][1], sep = ' ')
    
stplot <- ggplot() +
    geom_polygon(data = fortify(india.red[india.red$ST_NM == st_codes$state[st_codes$SUBNATIONAL1_CODE == current.state],]), aes(x=long, y=lat, group=group), fill = rgb(171/255, 215/255, 230/255), col = 'black') +
...
pp <- region.peeps %>% filter(SUBNATIONAL1_CODE == current.state) %>% slice(1:15) %>% ungroup %>% select(-SUBNATIONAL1_CODE)
  colnames(pp) <- c('Name','Lists','Hours')
  pp$Name <- strtrim(pp$Name,30)
  
region.spp.st <- region.spp %>% filter(SUBNATIONAL1_CODE == current.state) %>% arrange(perc.lists)
region.spp.st$Species <- do.call("rbind", strsplit(region.spp.st$SPECIES.NAME, " | ", fixed=TRUE))[,1]
  
    
cumlt <- all %>% filter(upload.month <= current.month, SUBNATIONAL1_CODE == current.state) %>% group_by(upload.month) %>% summarise(nlists = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>% mutate(cumlists = cumsum(nlists)) %>% filter(upload.month > '2013-12', upload.month <= current.month)
    
    
png(paste('C:/Users/Abinand Reddy/Desktop/Thesis/NCF/eBird/monthly summaries/monthly_report_',current.month,'_',current.state,'.png', sep = ''),width = 1300, height = 900, units = 'px')
plot.new()
grid.newpage()
...
pushViewport(vpstat)
grid.text(region.stats$total.lists[region.stats$SUBNATIONAL1_CODE == current.state], x = unit(.125, 'npc'), y = unit(.5,'npc'), gp=gpar(fontsize = 35, col = 'red4', fontface = 'bold'))
grid.text('Lists', x = unit(.125, 'npc'), y = unit(.2,'npc'), gp=gpar(fontsize = 20, col = 'black'))
grid.text(region.stats$total.recs[region.stats$SUBNATIONAL1_CODE == current.state], x = unit(.375, 'npc'), y = unit(.5,'npc'), gp=gpar(fontsize = 35, col = 'red4', fontface = 'bold'))
grid.text('Observations', x = unit(.375, 'npc'), y = unit(.2,'npc'), gp=gpar(fontsize = 20, col = 'black'))
grid.text(region.stats$total.peeps[region.stats$SUBNATIONAL1_CODE == current.state], x = unit(.625, 'npc'), y = unit(.5,'npc'), gp=gpar(fontsize = 35, col = 'red4', fontface = 'bold'))
grid.text('eBirders', x = unit(.625, 'npc'), y = unit(.2,'npc'), gp=gpar(fontsize = 20, col = 'black'))
grid.text(region.stats$total.sp[region.stats$SUBNATIONAL1_CODE == current.state], x = unit(.875, 'npc'), y = unit(.5,'npc'), gp=gpar(fontsize = 35, col = 'red4', fontface = 'bold'))
...
upViewport()
pushViewport(vp0)
grid.text(st_codes$state[st_codes$SUBNATIONAL1_CODE == current.state][1],x = unit(.6,'npc'),y = unit(.4, "npc"), gp=gpar(fontsize = 35, col = 'red4', fontface = 'bold'))
